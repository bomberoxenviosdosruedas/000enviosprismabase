...
import { Phone, Mail, MapPin, ChevronUp, Bike } from "lucide-react"
...
export function Footer() {
  ...
  const socialLinks = [
    { label: "Facebook", iconSrc: "/icons/facebook.svg", href: "https://facebook.com/enviosdosruedas" },
    { label: "Instagram", iconSrc: "/icons/instagram.svg", href: "https://instagram.com/enviosdosruedas" },
    { label: "WhatsApp", iconSrc: "/icon/icon-whatsapp.svg", onClick: handleWhatsAppClick, href: "#" },
  ]
              ...
              {socialLinks.map((social, index) => {
                return (
                  <motion.a
                    ...
                    className="p-3 rounded-full bg-primary-foreground/10 text-primary-foreground/80 hover:bg-secondary/20 hover:text-secondary border border-primary-foreground/10 hover:border-secondary/30 transition-all duration-300 backdrop-blur-sm flex items-center justify-center w-11 h-11"
                    aria-label={`${social.label} de Envios DosRuedas`}
                  >
                    <Image src={social.iconSrc} alt={social.label} width={20} height={20} className="w-5 h-5 invert dark:invert-0 brightness-0 dark:brightness-100" />
                  </motion.a>
                );
              })}
            </div>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center gap-4 font-sans">
            <div className="flex items-center gap-4">
              <p className="text-sm text-primary-foreground/70">
               ...
               <Link href="/repartidor" className="text-primary-foreground/50 hover:text-secondary transition-colors" aria-label="Acceso de repartidores">
